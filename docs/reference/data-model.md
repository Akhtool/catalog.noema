...
Основан на `reference/product-contract.md`.
...
- Поле `is_active` поддерживает скрытие бренда из активного списка.
- Текущая admin-реализация также поддерживает физическое удаление бренда; при этом `product.brand_id` очищается через `ON DELETE SET NULL`.
...
- Требование «минимум одно изображение на товар» сейчас не обеспечивается на уровне БД и не должно считаться жёстким schema-invariant.
...
данного файла reference/data-model.md
